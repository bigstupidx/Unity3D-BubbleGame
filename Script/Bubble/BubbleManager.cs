...
public class BubbleManager : MonoBehaviour {
	...
	{
		List<BubbleUnit> resList = new List<BubbleUnit> ();//待消除的bubble
		List<BubbleUnit> tempList = new List<BubbleUnit> ();

		tempList.AddRange (bubbleList);//存放所有bubble
		...
		resList.Add (bubble);//存放自身
		tempList.Remove (bubble);//存放与bubble不相同的
		int i = 0;
	...
	Transform GetBubbleTran(int ID)
	{
		...
		int type = IDTool.GetType (ID);//得到类型
		if (type == 1) {
			return pool.Spawn("BubbleUnit");//基础
		}
		if (type == 2) {
			return pool.Spawn("SpecialBubbleUnit");//爆炸
		}
		if (type == 3) {
			return pool.Spawn("FreezeBubbleUnit");//冷冻
		}
		if (type == 4) {
			return pool.Spawn("StoneBubbleUnit");//悬停
		}
